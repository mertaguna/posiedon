...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      className="mt-10 w-full bg-background font-sans dark:bg-neutral-950 md:px-10"
      ref={containerRef}
    >
      <div className="mx-auto w-full max-w-7xl px-4 py-2 pt-4 md:px-8 lg:px-10">
        <h3 className="max-w-4xl font-medium text-primary md:text-3xl">
          Our journey
        </h3>
      </div>
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              ...
              <h3 className="hidden font-sans text-xl font-medium text-foreground md:block md:pl-20 md:text-5xl">
                {item.title}
              </h3>
            </div>
